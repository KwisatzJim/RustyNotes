...
pub(crate) fn normalize_server_url(input: &str) -> Result<String, String> {
    ...
    let mut url = Url::parse(input).map_err(|_| "Enter a complete HTTPS server address.".to_string())?;
    if url.scheme() != "https" || url.host_str().is_none() || !input.to_lowercase().starts_with("https://") {
...
fn initialize(connection: &Connection) -> rusqlite::Result<()> {
    connection.execute_batch("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT NOT NULL);")
}

fn read_url(connection: &Connection) -> rusqlite::Result<Option<String>> {
    initialize(connection)?;
    connection.query_row("SELECT value FROM settings WHERE key = 'server_url'", [], |row| row.get(0)).optional()
}
...
pub fn get_server_url() -> Result<Option<String>, String> {
    let connection = crate::db::open_database().map_err(|_| "Could not open settings storage.".to_string())?;
    read_url(&connection).map_err(|_| "Could not read the server address.".to_string())
}

#[tauri::command]
pub async fn save_server_url(server_url: String, state: tauri::State<'_, crate::auth::LoginState>) -> Result<String, String> {
...
mod tests {
    ...
    fn normalizes_root_subdirectory_and_custom_port() {
        assert_eq!(normalize_server_url(" https://Cloud.Example.com ").unwrap(), "https://cloud.example.com/");
        assert_eq!(normalize_server_url("https://cloud.example.com:8443/nextcloud").unwrap(), "https://cloud.example.com:8443/nextcloud/");
        assert_eq!(normalize_server_url("https://cloud.example.com/nextcloud/").unwrap(), "https://cloud.example.com/nextcloud/");
    }

    #[test]
    fn rejects_unsafe_or_incomplete_addresses() {
        for input in ["", "cloud.example.com", "http://cloud.example.com", "file:///tmp", "https:cloud.example.com", "https://user:secret@cloud.example.com", "https://cloud.example.com/?token=secret", "https://cloud.example.com/#notes", "https://cloud.example.com/a b", "https://cloud.example.com\\notes"] {
    ...
    fn settings_round_trip_and_rejected_update_preserves_previous_value() {
        ...
        assert_eq!(read_url(&connection).unwrap().unwrap(), "https://cloud.example.com/nextcloud/");
        write_url(&connection, "https://other.example.com").unwrap();
        assert_eq!(read_url(&connection).unwrap().unwrap(), "https://other.example.com/");
    }
}
