...
pub fn store_batch(connection: &mut Connection, server: &str, account: &str, notes: &[RemoteNote]) -> Result<ImportSummary, String> {
    ...
    for note in notes {
        ...
        let snapshot = serde_json::to_string(note).map_err(|_| "Could not preserve the server snapshot.".to_string())?;
        transaction.execute(
    ...
    transaction.commit().map_err(|_| "Could not commit the import. Please retry.".to_string())?;
    Ok(summary)
}
...
mod tests {
    ...
    fn note(id: i64) -> RemoteNote {
        RemoteNote { id, title: "Title".into(), content: "# Markdown\n\n**hello** 📝".into(), category: "Work/Project".into(), favorite: true, modified: 123, etag: Some("original-etag".into()), readonly: Some(true) }
    }
    fn count(connection: &Connection) -> i64 {
        connection.query_row("SELECT COUNT(*) FROM notes", [], |r| r.get(0)).unwrap()
    }
    #[test]
    fn imports_fields_and_keeps_original_server_metadata() {
        ...
        let fields: (String, String, String, bool, i64) = connection.query_row(
            "SELECT title, content, category, favorite, modified_at FROM notes", [],
            |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?)),
        ).unwrap();
        assert_eq!(fields, (note(434).title, note(434).content, note(434).category, true, 123));
        let snapshot: String = connection.query_row("SELECT original_snapshot FROM imported_notes", [], |r| r.get(0)).unwrap();
    ...
    fn repeated_import_preserves_local_edits_and_does_not_duplicate() {
        ...
        connection.execute("UPDATE notes SET content = 'local edit'", []).unwrap();
        let summary = store_batch(&mut connection, "https://cloud/", "jim", &[note(1)]).unwrap();
        assert_eq!((summary.added, summary.skipped), (0, 1));
        let content: String = connection.query_row("SELECT content FROM notes", [], |r| r.get(0)).unwrap();
    ...
    fn unrelated_local_notes_and_account_namespaces_are_preserved() {
        let mut connection = database();
        connection.execute("INSERT INTO notes (title, content, modified_at) VALUES ('Title', 'local only', 1)", []).unwrap();
        for (server, account) in [("https://one/", "jim"), ("https://one/", "other"), ("https://two/", "jim")] {
            store_batch(&mut connection, server, account, &[note(1)]).unwrap();
        }
        assert_eq!(count(&connection), 4);
        let original: String = connection.query_row("SELECT content FROM notes WHERE id = 1", [], |r| r.get(0)).unwrap();
    ...
    fn failure_rolls_back_every_insert() {
        ...
        let mut bad = note(2); bad.title = "fail".into();
        assert!(store_batch(&mut connection, "https://cloud/", "jim", &[note(1), bad]).is_err());
        assert_eq!(count(&connection), 0);
        assert_eq!(store_batch(&mut connection, "https://cloud/", "jim", &[note(1)]).unwrap().added, 1);
    }
    #[test]
    ...
    fn empty_batch_is_safe_and_duplicates_rollback() {
        let mut connection = database();
        assert_eq!(store_batch(&mut connection, "https://cloud/", "jim", &[]).unwrap().added, 0);
        assert!(store_batch(&mut connection, "https://cloud/", "jim", &[note(1), note(1)]).is_err());
        assert_eq!(count(&connection), 0);
    }
}
